"""
project_2group_xs_ACCIDENT_noH_generator.py

Accident 2-group cross-section generator for the ME388D microreactor project.

Purpose:
    Generate accident fuel cross sections for a no-moderator/no-hydrogen fuel case.

Accident assumption:
    Hydrogen is removed from the U-ZrH1.6 fuel composition to represent loss of
    the fuel-bound moderator contribution. The remaining Table 3 fuel weight
...
Output:
    project_2group_xs_ACCIDENT_noH.json

Material basis:
    Fuel:      TRIGA progression problem Table 3 U-ZrH1.6 composition, with H-1 removed
               and all remaining weight fractions renormalized.
...
def create_uzrh_fuel_ACCIDENT_noH(lib):
    """
    Accident fuel material based on TRIGA Table 3 U-ZrH1.6 values with H-1 removed.

    The H-1 term is removed to represent a no-moderator fuel-region accident
    sensitivity case. The remaining weight fractions are renormalized so that
    the composition still sums to 1.0.
    """
    rho = 5.85
    fuel = Material("U-ZrH Fuel ACCIDENT no H moderator", temperature=900.0, density=rho)

    # TRIGA Table 3 U-ZrH1.6 composition with H-1 intentionally removed.
    table3_noH_weight_fractions_raw = [
        ("MN-55", 0.0014287),
        ("U-235", 0.0152),
        ("U-238", 0.061568),
        ("ZR-90", 0.43706),
    ...
    total_noH = sum(w for _, w in table3_noH_weight_fractions_raw)
    if total_noH <= 0.0:
        raise ValueError("No-H fuel weight fractions sum to zero or negative.")

    print("Accident no-H fuel case:")
    print("  Removed H-1 from TRIGA Table 3 fuel composition")
    print(f"  Raw remaining weight-fraction sum = {total_noH:.8f}")
    print("  Renormalizing remaining isotopes to sum to 1.0")

    for nuc_name, wt_frac_raw in table3_noH_weight_fractions_raw:
        wt_frac = wt_frac_raw / total_noH
        nuc = require_nuclide(lib, nuc_name)
...
def create_geometry(lib):
    """
    Half-core geometry for accident cross-section generation.
    ...
    Half-core dimensions:
        ...
        shield    = 100 cm
    """
    fuel = create_uzrh_fuel_ACCIDENT_noH(lib)
    reflector = create_graphite_reflector(lib)
    shield = create_stainless_steel_shield(lib)

    geom = OneDimensionalCartesianGeometryAndMesh("ACCIDENT no-H Microreactor Half-Core")
...
def main():
    ...
    print_block("ACCIDENT no-H 2-GROUP XS: FUEL", xs_fuel)
    print_block("ACCIDENT no-H 2-GROUP XS: REFLECTOR", xs_reflector)
    print_block("ACCIDENT no-H 2-GROUP XS: SHIELD", xs_shield)
    print_block("ACCIDENT no-H 2-GROUP XS: HOMOGENIZED TOTAL", xs_total)

    out = {
        "case": "ACCIDENT_noH",
        "description": "Accident 2-group cross sections generated by removing H-1 from the TRIGA Table 3 U-ZrH1.6 fuel composition and renormalizing the remaining fuel weight fractions.",
        "material_sources": {
            "fuel": "TRIGA progression problem Table 3 U-ZrH1.6, with H-1 removed for no-moderator accident case and remaining weight fractions renormalized",
        ...
        "accident_assumption": {
            "removed_from_fuel": "H-1",
            "renormalized_remaining_weight_fractions": True,
            "purpose": "Approximate fuel-region no-moderator sensitivity cross sections for accident modeling",
        },
        "group_convention": {
    ...
    out_name = "project_2group_xs_ACCIDENT_noH.json"
