# project_2group_xs_generator.py
#
# Standalone 2-group cross-section generator for the ME388D microreactor project.
# This file is built from the exact class/code cells in ME388D_HW3:
#   Cell 1: MPACTLibrary reader
#   Cell 2: S_N transport solver
#   Cell 3: Material class
#   Cell 4: 1D geometry/mesh
#   Cell 5: CrossSectionPostProcessor
#   Cell 6: Pincell generator (kept for reference)
#
# Put this script in the same directory as:
#   mpact_data_8g.fmt
#
# Then run:
#   python project_2group_xs_generator.py
#
# The project-specific part starts near the bottom under:
#   "PROJECT MICROREACTOR INPUTS"
#
...
def require_nuclide(lib, names):
    """
    Find a nuclide using a list of possible names.
    This helps because the MPACT library may use names like 'C-NAT' or 'C-12'.
    """
    ...
    available = [name for _, name in lib.list_nuclides()]
    raise ValueError(
        f"Could not find any of {names} in the library.\n"
        f"First available names are: {available[:60]}"
    )


def number_density_from_weight_fraction(density_g_cm3, weight_fraction, atomic_mass_g_mol):
    """
    Convert material density and weight fraction into number density.

    Number density units expected by HW3 Material.add_nuclide:
        atoms / barn-cm

    Formula:
        N = rho * w / A * N_A * 1e-24
    """
    return density_g_cm3 * weight_fraction / atomic_mass_g_mol * AVOGADRO * 1.0e-24


def add_by_weight_fraction(material, nuclide, density_g_cm3, weight_fraction):
    """
    Add one nuclide to a material using weight fraction.
    """
    N = number_density_from_weight_fraction(
        density_g_cm3=density_g_cm3,
        weight_fraction=weight_fraction,
        atomic_mass_g_mol=nuclide.atomic_mass
    )
    material.add_nuclide(nuclide, N, weight_fraction=weight_fraction)


def create_uzrh_fuel(lib):
    """
    Project baseline fuel: TRIGA-style UZrH fuel.

    These are starting composition assumptions based on the fuel mixture
    ...
    Density: 6.128 g/cm^3

    If your team chooses a different fuel, change this function.
    """
    u235 = require_nuclide(lib, "U-235")
    u238 = require_nuclide(lib, "U-238")
    h1   = require_nuclide(lib, "H-1")
    zr   = require_nuclide(lib, ["ZR-NAT", "ZR-90", "ZR-91", "ZR-92", "ZR-94", "ZR-96"])

    fuel = Material("UZrH Fuel", temperature=900.0, density=6.128)

    add_by_weight_fraction(fuel, u235, density_g_cm3=6.128, weight_fraction=0.02376)
    add_by_weight_fraction(fuel, u238, density_g_cm3=6.128, weight_fraction=0.09619)
    add_by_weight_fraction(fuel, h1,   density_g_cm3=6.128, weight_fraction=0.03000)
    add_by_weight_fraction(fuel, zr,   density_g_cm3=6.128, weight_fraction=0.85000)
...
def create_graphite_reflector(lib):
    """
    Project baseline reflector: graphite.
    Density: 1.70 g/cm^3, a reasonable engineering estimate.
    """
    c = require_nuclide(lib, ["C-NAT", "C-12"])

    graphite = Material("Graphite Reflector", temperature=600.0, density=1.70)
    add_by_weight_fraction(graphite, c, density_g_cm3=1.70, weight_fraction=1.0)

    return graphite


def create_borated_steel_shield(lib):
    """
    Project baseline shield: homogenized borated steel.

    This is a simplified/fantasy engineering material for the class model:
        Fe:   95 wt%
        B-10:  1 wt%
        B-11:  4 wt%
    ...
    Purpose:
        Fe gives structural/gamma shielding behavior.
        Boron gives strong neutron absorption.

    If the library does not include natural Fe, try common Fe isotopes.
    """
    fe = require_nuclide(lib, ["FE-NAT", "FE-56", "FE-54", "FE-57"])
    b10 = require_nuclide(lib, "B-10")
    b11 = require_nuclide(lib, "B-11")

    shield = Material("Borated Steel Shield", temperature=600.0, density=7.5)

    add_by_weight_fraction(shield, fe,  density_g_cm3=7.5, weight_fraction=0.95)
    add_by_weight_fraction(shield, b10, density_g_cm3=7.5, weight_fraction=0.01)
    add_by_weight_fraction(shield, b11, density_g_cm3=7.5, weight_fraction=0.04)

    return shield


def create_project_microreactor_slab(lib):
    """
    Creates the 1D half-core geometry for the XS-generation calculation.
    ...
    This is the same design scale we have been using for the diffusion model.
    """
    fuel = create_uzrh_fuel(lib)
    reflector = create_graphite_reflector(lib)
    shield = create_borated_steel_shield(lib)

    geom = OneDimensionalCartesianGeometryAndMesh("Project Microreactor Slab")

    # Use fewer cells if it runs slowly; use more if you want smoother flux weighting.
...
def xs_to_project_dict(xs):
    """
    Convert HW3 CollapsedXS object to a simple dictionary Aaron can plug into code.

    Convention:
        Group 1 = fast
        Group 2 = thermal

    scatter_matrix[from_group, to_group]
        [0, 1] = fast -> thermal downscatter
        [1, 0] = thermal -> fast upscatter
    """
    out = {
        "D": (1.0 / (3.0 * xs.sigma_tr)).tolist(),
...
def print_xs_block(label, xs):
    d = xs_to_project_dict(xs)
    print("\n" + "="*80)
    print(label)
    print("="*80)
    print("Group convention: group 1 = fast, group 2 = thermal")
    print(f"D       = {d['D']}")
    ...
    print(f"chi     = {d['chi']}")
    print("scatter matrix [from_group][to_group] =")
...
def main():
    mpact_path = Path("mpact_data_8g.fmt")

    if not mpact_path.exists():
        raise FileNotFoundError(
            "Could not find mpact_data_8g.fmt. Put this script in the same "
            "folder as mpact_data_8g.fmt, or edit mpact_path in main()."
        )

    lib = MPACTLibrary(str(mpact_path))

    geom, materials = create_project_microreactor_slab(lib)
    ...
    # Region order follows create_project_microreactor_slab():
    #   0 = fuel
    #   1 = reflector
    #   2 = shield
    ...
    print_xs_block("2-GROUP XS: FUEL REGION", xs_fuel)
    print_xs_block("2-GROUP XS: REFLECTOR REGION", xs_reflector)
    print_xs_block("2-GROUP XS: SHIELD REGION", xs_shield)
    print_xs_block("2-GROUP XS: HOMOGENIZED TOTAL", xs_total)
    ...
    with open("project_2group_xs.json", "w") as f:
        json.dump(project_xs, f, indent=2)

    print("\nSaved: project_2group_xs.json")
